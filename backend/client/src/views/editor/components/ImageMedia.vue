<template>
  <div>
    <div>
      <img class="output" ref="output" :src="elem.url" else />
      <div class="img" v-if="elem.url == ''">
        <!-- v-if="elem.type == 'img'" -->
        ...
        <form
          ...
          @dragover.prevent
        >
          <label :for="'upload' + index" class="choose"></label>
          <i class="far fa-image"></i>

          <div>
            <label class="chooselabel" :for="'upload' + index"
              >Choose a file
            </label>
      ...
      try {
        await axios
          ...
          .then((res) => {
            ...
            this.updateUrl({ url: res.data.path, stateIndex: this.index });
          });
      } catch (err) {
        console.log(err);
